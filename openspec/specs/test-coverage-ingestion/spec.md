...
Define how SovereignID produces test coverage reports (Coverlet OpenCover +
Cobertura), applies exclusions in layers, enforces per-bounded-context floors in
CI, integrates with SonarCloud (`Sonar way`), consolidates GitHub Actions
workflows, and documents the `Integration` test category convention.
...
traceable in repository docs (`README.md`, `docs/onboarding.md`) or the
archived change design notes.
